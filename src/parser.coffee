...
genAst = (o) ->
	...
	switch o[0]
		...
		when "function"
			...
			return node("func-literal", ln, closure: node("closure-context", ln, name: name, args: args, stat: node("block-stat", ln, stats: (genAst(x) for x in stats))))
		when "new"
			[_, ln, func, args] = o
		...
		when "toplevel"
			[_, ln, stats] = o
			return node("script-context", ln, stat: node("block-stat", ln, stats: (genAst(x) for x in stats)))
		when "block"
			[_, ln, stats] = o
			stats = stats or []
		...
		when "defun"
			[_, ln, name, args, stats] = o
			return node("defn-stat", ln, closure: node("closure-context", ln, name: name, args: args, stat: node("block-stat", ln, stats: (genAst(x) for x in stats))))
		when "return"
			[_, ln, value] = o
